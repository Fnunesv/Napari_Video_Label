[← Back to tutorial index](tutorial.md)

# 5. Scale bar

Draws a physical-length scale bar (and its label) on every frame, sized
using the **Pixel size** from section 2.

| Field | Meaning |
|---|---|
| Show scale bar | Toggle on/off |
| Length (µm) | Physical length the bar represents |
| Offset from corner (px) | X/Y distance inward from the anchor corner |
| Label font size | Size of the length label drawn next to the bar |
| Color | Applies to both the bar and its label |

<p align="center">
  <img src="images/04-timestamp-scalebar.png" width="500" alt="Scale bar section">
</p>

<!--
SCREENSHOT NEEDED: images/05-scale-bar.png
Section 5 group box, plus a preview frame showing the scale bar rendered
in a corner with its label.
-->

If you change the **Pixel size** in section 2, the bar's on-screen length
updates automatically to keep representing the same physical distance.
